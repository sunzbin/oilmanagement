...
public class AscIIController extends JavaEEFrameworkBaseController<AscII> implements Constant{
	...
	// 查询字典的表格，包括分页、搜索和排序
	@RequestMapping(value = "/getAscii", method = { RequestMethod.POST, RequestMethod.GET })
	public void getAscii(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		QueryResult<AscII> queryResult = ascIIService.doPaginationQuery(dict);
		JqGridPageView<AscII> dictListView = new JqGridPageView<AscII>();
	...
	public void doSave(AscII entity, HttpServletRequest request, HttpServletResponse response) throws IOException {
		ExtJSBaseParameter parameter = ((ExtJSBaseParameter) entity);
		if (CMD_EDIT.equals(parameter.getCmd())) {
			ascIIService.merge(entity);
		} else if (CMD_NEW.equals(parameter.getCmd())) {
			ascIIService.persist(entity);
		}
	...
	@RequestMapping(value = "/operateAscii", method = { RequestMethod.POST, RequestMethod.GET })
	public void operateDict(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (oper.equals("del")) {
			...
			AscII dict = null;
			if (oper.equals("edit")) {
				dict = ascIIService.get(Long.valueOf(id));
			}
			AscII entity = new AscII();
			entity.setBin(request.getParameter("bin"));
			entity.setQoc(request.getParameter("qoc"));
			entity.setDec(request.getParameter("dec"));
			entity.setHex(request.getParameter("hex"));
			entity.setCharacters(request.getParameter("characters"));
			entity.setExplain(request.getParameter("explain"));
			if (oper.equals("edit")) {
	...
	public void deleteDict(HttpServletRequest request, HttpServletResponse response, @RequestParam("ids") Long[] ids) throws IOException {
		boolean flag = ascIIService.deleteByPK(ids);
		if (flag) {
